 ...
 * supporting auto-partition assignment or user-configured assignment.
 * <p>
 ...
 * @author Artem Bilan
 */
public class WSMPKafkaMessageListenerContainer<K, V> extends AbstractMessageListenerContainer<K, V> {
	 ...
	 */
	public WSMPKafkaMessageListenerContainer(ConsumerFactory<K, V> consumerFactory,
			ContainerProperties containerProperties) {
		this(consumerFactory, containerProperties, (TopicPartitionInitialOffset[]) null);
	}

	/**
	 * Construct an instance with the supplied configuration properties and
	 * specific topics/partitions/initialOffsets.
	 * 
	 * @param consumerFactory
	 *            the consumer factory.
	 * @param containerProperties
	 ...
	 *            the topics/partitions; duplicates are eliminated.
	 */
	public WSMPKafkaMessageListenerContainer(ConsumerFactory<K, V> consumerFactory,
		...
		if (isRunning()) {
			this.listenerConsumerFuture.addCallback(new ListenableFutureCallback<Object>() {
				...
				public void onFailure(Throwable e) {
					WSMPKafkaMessageListenerContainer.this.logger.error("Error while stopping the container: ", e);
					if (callback != null) {
						callback.run();
					}
				}
				...
				public void onSuccess(Object result) {
					if (WSMPKafkaMessageListenerContainer.this.logger.isDebugEnabled()) {
						WSMPKafkaMessageListenerContainer.this.logger
								.debug(WSMPKafkaMessageListenerContainer.this + " stopped normally");
					}
					if (callback != null) {
						callback.run();
					}
	...
	private final class ListenerConsumer implements SchedulingAwareRunnable {
		...
		private final OffsetCommitCallback commitCallback = this.containerProperties.getCommitCallback() != null
				? this.containerProperties.getCommitCallback() : new LoggingCommitCallback();

		private final Consumer<K, V> consumer;

		private final Map<String, Map<Integer, Long>> offsets = new HashMap<>();

		protected final MessageListener<K, V> listener;

		protected final AcknowledgingMessageListener<K, V> acknowledgingMessageListener;

		private final boolean autoCommit = WSMPKafkaMessageListenerContainer.this.consumerFactory.isAutoCommit();
		...
		/**
		 * The consumer is currently paused due to a slow listener. The consumer
		 * will be resumed when the current batch of records has been processed
		 * but will continue to be polled.
		 */
		...
		private ListenerConsumer(MessageListener<K, V> listener, AcknowledgingMessageListener<K, V> ackListener) {
			...
			final Consumer<K, V> consumer = WSMPKafkaMessageListenerContainer.this.consumerFactory.createConsumer();
			...
			ConsumerRebalanceListener rebalanceListener = new ConsumerRebalanceListener() {
				...
				public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
					...
					if (!ListenerConsumer.this.autoCommit) {
						if (ListenerConsumer.this.logger.isTraceEnabled()) {
							ListenerConsumer.this.logger.trace(
									"Received partition revocation notification, " + "and will stop the invoker.");
						}
				...
				public void onPartitionsAssigned(Collection<TopicPartition> partitions) {
					...
					if (!ListenerConsumer.this.autoCommit) {
						// Commit initial positions - this is generally
						// redundant but
						// it protects us from the case when another consumer
						// starts
						// and rebalance would cause it to reset at the end
						// see
						// https://github.com/spring-projects/spring-kafka/issues/110
						...
						if (WSMPKafkaMessageListenerContainer.this.getContainerProperties().isSyncCommits()) {
							ListenerConsumer.this.consumer.commitSync(offsets);
						} else {
							ListenerConsumer.this.consumer.commitAsync(offsets, WSMPKafkaMessageListenerContainer.this
									.getContainerProperties().getCommitCallback());
						}
					}
					// We will not start the invoker thread if we are in
					// autocommit mode,
					// as we will execute synchronously then
					// We will not start the invoker thread if the container is
					// stopped
					// We will not start the invoker thread if there are no
					// partitions to
					// listen to
					if (!ListenerConsumer.this.autoCommit && WSMPKafkaMessageListenerContainer.this.isRunning()
			...
			if (WSMPKafkaMessageListenerContainer.this.topicPartitions == null) {
				...
				List<TopicPartitionInitialOffset> topicPartitions = Arrays
						.asList(WSMPKafkaMessageListenerContainer.this.topicPartitions);
		...
		public void run() {
			...
			if (isRunning() && this.definedPartitions != null) {
				...
				// we start the invoker here as there will be no rebalance calls
				// to
				// trigger it, but only if the container is not set to
				// autocommit
			...
			while (isRunning()) {
				try {
					...
					if (records != null && records.count() > 0) {
						...
						// if the container is set to auto-commit, then execute
						// in the
						// same thread
						...
						if (this.autoCommit) {
							...
							if (sendToListener(records)) {
								if (this.assignedPartitions != null) {
									// avoid group management rebalance due to a
									// slow
									// consumer
		...
		private void publishIdleContainerEvent(long idleTime) {
			if (this.applicationEventPublisher != null) {
				this.applicationEventPublisher.publishEvent(new ListenerContainerIdleEvent(
						WSMPKafkaMessageListenerContainer.this, idleTime, getBeanName(), getAssignedPartitions()));
			}
		}
		...
		private ConsumerRecords<K, V> checkPause(ConsumerRecords<K, V> unsent) {
			if (this.paused && this.recordsToProcess.size() < this.containerProperties.getQueueDepth()) {
				...
				this.consumer
						.resume(this.assignedPartitions.toArray(new TopicPartition[this.assignedPartitions.size()]));
				this.paused = false;
				if (unsent != null) {
					try {
		...
		private void ackImmediate(ConsumerRecord<K, V> record) {
			...
			if (this.containerProperties.isSyncCommits()) {
				...
				ListenerConsumer.this.consumer.commitAsync(commits, ListenerConsumer.this.commitCallback);
			}
		}
					...
					if (ackMode.equals(AckMode.TIME) && elapsed) {
						...
						if (this.logger.isDebugEnabled()) {
							if (elapsed) {
								...
								this.logger.debug("Committing in AckMode.COUNT_TIME " + "because count " + this.count
										+ " exceeds configured limit of" + this.containerProperties.getAckCount());
							}
						}
		...
		private void initPartitionsIfNeeded() {
			/*
			 * Note: initial position setting is only supported with explicit
			 * topic assignment. When using auto assignment (subscribe), the
			 * ConsumerRebalanceListener is not called until we poll() the
			 * consumer.
			 */
			for (Entry<TopicPartition, Long> entry : this.definedPartitions.entrySet()) {
				...
				if (offset != null) {
					...
					try {
						...
						logger.error("Failed to set initial offset for " + topicPartition + " at " + newOffset
								+ ". Positioned to " + this.consumer.position(topicPartition), e);
					}
				}
			}
		}
