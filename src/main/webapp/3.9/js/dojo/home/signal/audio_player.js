...
MmPlayer.prototype = {
    ...
    _startApp: function() {
        ...
        dropContainer.addEventListener("dragleave", function() {
            if (that.status === 1) {
                that._updateTitle('正在播放 ' + that.currentFileName, false);
            } else {
    ...
    _readFile: function(orderNum) { //read file as arraybuffer
        ...
        reader.onerror = function(e) {
            that._updateTitle('读取文件失败！', false);
        this.processing=false;
            console.log(e);
        };
    ...
    _drawSpectrum: function(audioCtx, buffer) {
        ...
        source.start(0);
        this.processing=false;
        this._updateTitle('正在播放 ' + this.playlist[this.currentOrderNum].name.slice(0, -4), false);
        this.source = source;
        this.status = 1;
    ...
    drawMarkCat: function() {
        var that = this,
            imgPath = 'images/mmplayer_mosaik.png',
            img = new Image();
        img.onload = function() {
            var c = that.canvas,
