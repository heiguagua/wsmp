        ...
        <div class='item feature' style="width: 100%;">
          ...
          <!--     <li><div class='item-name' title="谱密度最大值">谱密度最大值</div>
              ...
              <li>
                ...
                <div class='item-value' title="${redioDetail.rmax}">${redioDetail.rmax}</div>
              </li>
              <li>
                <div class='item-name' title="频谱的周期性">频谱的周期性</div>
                <div class='item-value' title="${redioDetail.specT}">${redioDetail.specT}</div>
              </li>
              <li>
                <div class='item-name' title="码速率">码速率</div>
                <div class='item-value' title="${redioDetail.symRate}">${redioDetail.symRate}</div>
              </li>
              <li>
                <div class='item-name' title="频率标准偏差">频率标准偏差</div>
                <div class='item-value' title="${redioDetail.deltaAF}">${redioDetail.deltaAF}</div>
              </li>
              <li>
                <div class='item-name' title="功率谱平稳度指数">功率谱平稳度指数</div>
                <div class='item-value' title="${redioDetail.flatDegree}">${redioDetail.flatDegree}</div>
              </li>
              <li>
                <div class='item-name' title="频率峰值个数">频率峰值个数</div>
                <div class='item-value' title="${redioDetail.freqPeakNumFSK}">${redioDetail.freqPeakNumFSK}</div>
              </li>
              <li>
                <div class='item-name' title="幅度谱峰值比">幅度谱峰值比</div>
                <div class='item-value' title="${redioDetail.aspecPeakAvgRate}">${redioDetail.aspecPeakAvgRate}</div>
              </li>
              <li>
                <div class='item-name' title="频谱谱峰数">频谱谱峰数</div>
                <div class='item-value' title="${redioDetail.spectrumPeak}">${redioDetail.spectrumPeak}</div>
              </li>
              <li>
                <div class='item-name' title="相位过零点标准差">相位过零点标准差</div>
                <div class='item-value' title="${redioDetail.fstdZeroDis}">${redioDetail.fstdZeroDis}</div>
              </li>
              <li>
                <div class='item-name' title="保留">保留</div>
                <div class='item-value' title="${redioDetail.ampCorrMaxrate}">${redioDetail.ampCorrMaxrate}</div>
              </li>
              <li>
                <div class='item-name' title="IQ最大值">IQ最大值</div>
                <div class='item-value' title="${redioDetail.maxIQ}">${redioDetail.maxIQ}</div>
              </li>
              <li>
                <div class='item-name' title="幅度的周期">幅度的周期</div>
                <div class='item-value' title="${redioDetail.ampT}">${redioDetail.ampT}</div>
              </li>
              <li>
                <div class='item-name' title="相位统计个数">相位统计个数</div>
                <div class='item-value' title="${redioDetail.phaseNum}">${redioDetail.phaseNum}</div>
              </li>
              <li>
                <div class='item-name' title="频率零值波峰比">频率零值波峰比</div>
                <div class='item-value' title="${redioDetail.fzeropeakRate}">${redioDetail.fzeropeakRate}</div>
              </li>
              <li>
                <div class='item-name' title="频率谱的平稳度">频率谱的平稳度</div>
                <div class='item-value' title="${redioDetail.fspecFlagDegree}">${redioDetail.fspecFlagDegree}</div>
              </li>
              <li>
                <div class='item-name' title="频率谱的峰值比">频率谱的峰值比</div>
                <div class='item-value' title="${redioDetail.fspecPeakAvgRate}">${redioDetail.fspecPeakAvgRate}</div>
              </li>
              <li>
                <div class='item-name' title="码元跳变处相位差">码元跳变处相位差</div>
                <div class='item-value' title="${redioDetail.codePhase}">${redioDetail.codePhase}</div>
              </li>
              <li>
                <div class='item-name' title="频谱周期特征">频谱周期特征</div>
                <div class='item-value' title="${redioDetail.specTRate}">${redioDetail.specTRate}</div>
              </li>
              <li>
                <div class='item-name' title="频率频谱峰值比">频率频谱峰值比</div>
                <div class='item-value' title="${redioDetail.freqWidthRate}">${redioDetail.freqWidthRate}</div>
              </li>
              <li>
                <div class='item-name' title="时分信号比率">时分信号比率</div>
                <div class='item-value' title="${redioDetail.timedifRate}">${redioDetail.timedifRate}</div>
              </li>
              <li>
                <div class='item-name' title="GSM时隙比率">GSM时隙比率</div>
                <div class='item-value' title="${redioDetail.ampCorrRate}">${redioDetail.ampCorrRate}</div>
              </li>
            </ul>
          </div>
        </div>
